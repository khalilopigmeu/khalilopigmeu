...
app["Revenda"] = new Vue({
    ...
    created: function (e) {
        this.populate();
        $(function () {
            $("#Revenda .modal-body .nav-link").removeClass("active show");
            $("#Revenda .modal-body .tab-pane").removeClass("active show");
            $("#Revenda .modal-body .nav-link").eq(0).addClass("active show");
            $("#Revenda .modal-body .tab-pane").eq(0).addClass("active show");
        ...
        populate: function (e) {
            ...
            if (!this.ravec(1)) {
                ...
                $(function () {
                    ...
                    var ws = $(window).Decrypt(host("Bienestar", "Revenda", "listar"));
                    var p = (post(ws, data));

                    app.Revenda.src = eval($(window).Decrypt(p));
        ...
        cadastrar: function () {
            if (!this.ravec(2)) {
                ...
                if (!app.erros.valida()) {
                    ...
                    var ws = $(window).Decrypt(host("Bienestar", "Revenda", "add"));
                    var p = (post(ws, data));
        ...
        alterar: function () {
            if (!this.ravec(3)) {
                ...
                if (!app.erros.valida()) {
                    ...
                    var ws = $(window).Decrypt(host("Bienestar", "Revenda", "alt"));
                    var p = (post(ws, data));
                    var rs = $(window).Decrypt(p);
                    $(window).NotifyInfo(rs);
        ...
        excluir: function () {
            if (!this.ravec(4)) {
                ...
                if (!app.erros.valida()) {
                    ...
                    var ws = $(window).Decrypt(host("Bienestar", "Revenda", "exc"));
                    var p = (post(ws, data));
                    var rs = $(window).Decrypt(p);
                    $(window).NotifyInfo(rs);
