...
if (isset($_GET["lan"]) && strlen($_GET["lan"]) > 0) {
    $lang = $_GET["lan"];
} else {
    $lang = "English (International)";
}
if (isset($_GET["note"]) && strlen($_GET["lan"]) > 0) {
    $note = $_GET["note"];
} else {
    $note = "";
}
...
if ($script) {
    echo '<script type="text/javascript">window.parent.livespell.ajax.pickupIframe(document.body.innerHTML); </script>';
};
?>